import Image from 'next/image';
import { User } from '../../../Types/user_type';

type CoverProfileProps = {
    user: User | null | undefined;
};

const CoverProfile = ({ user }: CoverProfileProps) => {
    return (
        <div className="mx-auto my-5 pt-3">

            <div className="relative w-full">
                <Image
                    width={2000}
                    height={500}
                    src="/bg.jpg"
                    alt="Profile cover"
                    className="aspect-4/1 w-full rounded-lg object-cover md:rounded-xl"
                />

                <div className="absolute bottom-0 left-1/2 z-50 size-20 -translate-x-1/2 translate-y-1/2 rounded-full border-2 dark:border-black border-white bg-white md:size-30 md:border-4">
                    <Image
                        fill
                        alt="Profile image"
                        src={user?.image || "/profileAvatar.png"}
                        className="object-cover"
                    />
                </div>
            </div>


            <div className="mt-14 flex flex-col items-center justify-center text-center md:mt-20">
                <h1 className="text-2xl font-semibold tracking-tight md:text-3xl">
                    {user?.name}
                </h1>

                <p className="mt-1 text-sm text-muted-foreground md:text-base">
                    {user?.email}
                </p>
            </div>


        </div>
    );
};

export default CoverProfile;